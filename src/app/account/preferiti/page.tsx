...
function normalizeFavorites(json: any): Favorite[] {
  // Supporta più formati possibili:
  // - { ok:true, favorites:[...] }
  // - { ok:true, data:[...] }
  // - { favorites:[...] } (anche se non ok, ma capita)
  const raw =
  ...
  for (const item of raw) {
    ...
    if (productId == null || !slug || !name) {
      // Se manca qualcosa, non blocchiamo tutto: semplicemente saltiamo quell’elemento
      continue;
    }
...
async function fetchJsonTryEndpoints(
  ...
  for (const base of API_ENDPOINTS) {
    ...
    // Se l’endpoint non esiste (404), proviamo il successivo
...
export default function FavoritesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Standard atteso: { ok:true, favorites:[...] }
