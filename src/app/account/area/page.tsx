...
const NEXT_PATH = "/account/area";
const LOGIN_FALLBACK = "/accedi?next=/account/area";
...
export default async function AccountAreaPage() {
                      ...
                      {o.products.map((p) => (
                        <li key={String(p.id)} className="text-sm">
                          ...
                          <span className="text-text/60">
                            — prezzo listino € {(p.compareAtPrice ?? p.price).toFixed(2)}
                          </span>
                        </li>
                      ))}
                    </ul>
