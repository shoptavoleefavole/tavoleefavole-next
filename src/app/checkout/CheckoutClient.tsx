...
function addressSignature(a: Address) {
  return [a.fullName, a.street, a.cap, a.city, a.province ?? "", a.country]
    .map((v) => String(v ?? "").trim().toLowerCase())
    .join("|");
}

function mergeAddresses(localAddresses: Address[], profileAddresses: Address[]): Address[] {
...
export default function CheckoutClient({
  ...
  useEffect(() => {
    ...
    const merged = mergeAddresses(localAddresses, initialAddresses);
    setAddresses(merged);

    const localDefault = merged.find((a) => a.isDefault);
    if (localDefault?.id) setSelectedId(localDefault.id);
    else if (merged[0]?.id) setSelectedId(merged[0].id);
